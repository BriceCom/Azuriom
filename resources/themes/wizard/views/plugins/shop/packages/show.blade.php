...
<div class="modal-dialog modal-md modal-dialog-centered" role="document">
    <div class="modal-content">
        <div class="modal-header justify-content-center border-0">
            <h3 class="modal-title d-block fs-4" id="itemModalLabel">{{ $package->name }}</h3>
{{--            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>--}}
        </div>
        <div class="modal-body">
            <p>Prix:
                <span class="text-yellow-600 fw-medium mb-3">{{ shop_format_amount($package->getPrice()) }}</span>
            </p>
            {!! $package->description !!}
        </div>
        <div class="modal-footer d-flex flex-column text-center border-0">
            ...
            @auth
                ...
                @else
                    <form action="{{ route('shop.packages.buy', $package) }}" method="POST"
                          class="row row-cols-lg-auto g-0 gy-2 align-items-center">
                        @csrf

                        @if(mb_strtolower($package->category->name) !== 'grades')
                            @if($package->custom_price)
                                <label for="price">{{ trans('shop::messages.fields.price') }}</label>

                                <div class="mx-3">
                                    <input type="number" min="{{ $package->price }}" size="5" class="form-control"
                            ...
                            @if($package->has_quantity)
                                ...
                                <div class="mx-3">
                                    <input type="number" min="0" max="{{ $package->getMaxQuantity() }}" size="5"
                                           class="form-control" name="quantity" id="quantity" value="1" required>
                                </div>
                            @endif
                        @endif

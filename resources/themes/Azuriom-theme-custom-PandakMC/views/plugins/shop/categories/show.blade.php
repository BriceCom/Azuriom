    ...
    <div class="row gy-8" id="shop">
        ...
        <div class="col-12">
            <div class="row gy-10 gx-7">
                ...
                <div class="col-12">
                    <div class="d-flex flex-wrap justify-content-center package-wrapper">
                                    ...
                                    <div class="card-body d-flex flex-column align-items-center text-center justify-content-between gap-2 pt-0">
                                        <div>
                                            @php
                                                $number = null;
                                                $text = $package->name;
                                                (preg_match('/(\d+[,\.]?\d*)/', $package->name, $matches));
                                                if($matches){
                                                     $number = $matches[0];
                                                     $text = preg_replace('/\d+[,\.]?\d*/', '', $package->name);
                                                }
                                            @endphp


                                            <span class="d-flex align-center gap-2 card-title fw-bold h5 mb-1 text-nowrap">
                                                @if(strtolower($category->name) === 'gemmes')
                                                <img class="me-2 object-fit-contain" src="{{ theme_config('shop.index.art-image') ? image_url(theme_config('shop.index.art-image')) : theme_asset('images/illu_rubis.png') }}" width="20" alt="Rubis"/> @if($number) <span class="fw-bold text-primary">{{ $number }}</span> @endif
                                                @endif
                                                {{ $text }}
                                            </span>

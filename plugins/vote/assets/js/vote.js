if (window.__votePluginInitialized) {
    // Script already loaded once (theme + plugin stacks, cache mismatch, etc.).
} else {
    window.__votePluginInitialized = true;
...
function initVote() {
    document.querySelectorAll('[data-vote-url]').forEach(function (el) {
        ...
        const clickListener = function (ev) {
            ...
            const currentVoteTime = el.dataset['voteTime'];
            if ((currentVoteTime && currentVoteTime > Date.now()) || el.classList.contains('disabled')) {
                ev.preventDefault();
                return;
            }
...
function setupVoteTimers(name) {
    ...
    while (voteDoneCallbacks.length > 0) {
        const callback = voteDoneCallbacks.shift();

...
@section('content')
    ...
    @if(! $subscriptions->isEmpty())
        <div class="card mb-8">
            <div class="card-body">
                <h2 class="card-title mb-8">{{ trans('shop::messages.profile.subscriptions') }}</h2>
    ...
    <div class="card">
        @if(use_site_money())
            <div>
                <h2 class="mb-8">{{ trans('shop::messages.giftcards.add') }}</h2>

                <form action="{{ route('shop.giftcards.add') }}" method="POST" class="flex items-end gap-2">
                    @csrf

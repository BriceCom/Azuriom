@php
    ...
    $themeEditorLang = trans('theme::admin.theme_editor');
    if (! is_array($themeEditorLang)) {
        $themeEditorLang = [];
    }

    $adminThemeEditorLang = trans('admin.theme_editor');
    if (is_array($adminThemeEditorLang)) {
        $themeEditorLang = array_replace_recursive($themeEditorLang, $adminThemeEditorLang);
    }

    $editorLang = array_replace_recursive($editorLang, $themeEditorLang);
    $themeEditorText = $themeEditorLang;
    $te = static fn (string $key, string $fallback = '') => data_get($themeEditorText, $key, $fallback);
...
<div id="{{ $themeEditorRootId }}" data-theme-editor-root>
    <button id="themeEditorToggle" class="te-toggle" type="button" aria-controls="{{ $themeEditorPanelId }}" aria-expanded="false">
        {{ $te('common.edit', 'Éditer') }}
    </button>

    <aside id="{{ $themeEditorPanelId }}" class="te-panel" aria-hidden="true">
        <header class="te-header">
            <div>
                <h2 class="te-title">{{ $te('header.title', 'Theme Editor Live') }}</h2>
                <p id="teStatusText" class="te-status-text">{{ $te('header.ready', 'Prêt') }}</p>
            </div>
            <div class="te-header-actions">
                <span id="teStateBadge" class="te-state-badge">{{ $te('state.idle', 'Idle') }}</span>
                <button type="button" id="themeEditorClose" class="te-icon-btn" aria-label="{{ $te('common.close', 'Fermer') }}">×</button>
            </div>
        </header>

        <nav class="te-tabs" role="tablist" aria-label="{{ $te('tabs.aria_label', 'Theme Editor tabs') }}">
            <button type="button" class="te-tab te-tab-active" data-te-tab-target="styles">{{ $te('tabs.styles', 'Styles') }}</button>
            <button type="button" class="te-tab" data-te-tab-target="global">{{ $te('tabs.global', 'Global') }}</button>
            <button type="button" class="te-tab" data-te-tab-target="advanced">{{ $te('tabs.advanced', 'Mode avancé') }}</button>
            <button type="button" class="te-tab" data-te-tab-target="modules">{{ $te('tabs.modules', 'Modules') }}</button>
            <button type="button" class="te-tab" data-te-tab-target="variables">{{ $te('tabs.variables', 'Variables') }}</button>
            <button type="button" class="te-tab" data-te-tab-target="page">{{ $te('tabs.page', 'Page builder') }}</button>
        </nav>
        ...
        <footer class="te-footer">
            ...
            >
                ...
                <span>{{ $te('footer.support', 'Support') }}</span>
            </a>
            <button type="button" id="teCancel" class="te-btn te-btn-ghost">{{ $te('footer.cancel', 'Annuler') }}</button>
            <button type="button" id="teSave" class="te-btn te-btn-success">
                <span class="te-save-label">{{ $te('footer.save', 'Save') }}</span>
                <span class="te-save-loading" hidden>{{ $te('footer.saving', 'Sauvegarde…') }}</span>
            </button>
        </footer>
    </aside>

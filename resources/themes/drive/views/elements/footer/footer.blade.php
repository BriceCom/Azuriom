<footer class="drive-footer">
    <div class="drive-footer__inner">
        <div class="drive-footer__top">
            @include('components.socials')

            <div class="d-flex flex-wrap align-items-center gap-3">
                @include('components.join-button', ['variant' => 'secondary'])
                <a href="#app" class="btn btn-primary btn-icon">
                    <i class="bi bi-arrow-up"></i>
                </a>
            </div>
        </div>

        <div class="drive-footer__grid">
            <div class="drive-footer__brand">
                @php($footerImage = theme_config('footer.index.image'))
                <img src="{{ $footerImage ? image_url($footerImage) : site_logo() }}" alt="Logo {{site_name()}}" class="object-fit-contain" style="max-width: 220px"/>
                            ...
                            @if($link['links'])
                                @foreach($link['links'] as $sublink)
                                    @if($sublink['name'])
                                        <li>
                                            <a href="{{$sublink['href'] ?? "#"}}"
                                               class="nav-link fw-light text-lg py-1"
                                               @if(isset($sublink['target'])) target="_blank" @endif>
                                                <b>{{$sublink['name'] ?? ""}}</b>
                                            </a>
                                        </li>
                                    @endif
                ...
                <div class="drive-footer__cta">
                    ...
                    <a href="{{theme_config('footer.index.button.url') ?? "#"}}"
                       ...
                       rel="noopener noreferrer">
                        @if(theme_config('footer.index.button.icon'))
        ...
        <p class="drive-footer__legal mb-0">

        ...
        <div class="col-span-12">
            <div class="mx-auto max-w-2xl bg-steel-200 rounded-2xl">
                ...
                <div class="p-8 grid xl:grid-cols-2 grid-cols-1 gap-8">
                    <p class="col-span-2 bg-forest p-4 rounded-md text-white">{{ trans('messages.profile.2fa.enabled')
                        }}</p>


                    <div class="col-span-2 lg:col-span-1">
                        <p class="text-white">Voici vos codes de récupération :</p>
                        <a href="{{ route('profile.2fa.codes') }}"
                            class="py-2 px-4 inline-flex justify-center items-center gap-2 rounded-xl bg-primary text-white font-semibold transition-all text-sm truncate cursor-pointer w-full mt-4"
                            download="{{ $codesBackupName }}">
                            <i class="bi bi-download"></i> {{ trans('messages.actions.download') }}
                        </a>
                    </div>
                    @if($user->two_factor_recovery_codes !== null)
                    <ul class="bg-steel-300 text-white rounded-md text-center p-8 col-span-2 lg:col-span-1">
                    ...
                    <div class="col-span-2">
                        <form action="{{ route('profile.2fa.disable') }}" method="POST"
                            class="grid xl:grid-cols-2 grid-cols-1 gap-8">
                            @csrf

                            <a class="w-full py-4 px-5 inline-flex justify-center items-center gap-2 rounded-xl bg-steel-300 text-white font-semibold transition-all text-xs truncate"

...
<div class="form-group">
    ...
    <div class="d-flex align-center gap-2">
        ...
        <input id="{{ $idReplace }}" name="{{ $id }}" type="range" min="{{$min}}" max="{{$max}}" step="{{$step}}" class="form-range bg-body p-2 mb-2 @error($id) is-invalid @enderror" value="{{ old($idReplace, config('theme.'.str_replace(['[', ']'],['.',''],$id))) ?? $value }}" aria-describedby="{{$id}}" oninput="this.previousElementSibling.value = this.value">
    </div>
    @error($id)

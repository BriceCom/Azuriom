window.ThemeEditorCoreLayout = function createThemeEditorCoreLayout(context) {
        ...
        blockCatalog.forEach((item) => {
            ...
            article.innerHTML = `
                ...
                <div class="te-block-actions">
                    <button type="button" class="te-btn te-btn-success">Ajouter</button>
                </div>
            `;
            ...
            if (!isAddable || isUniqueAndAlreadyAdded) {
                ...
                addButton.textContent = isUniqueAndAlreadyAdded ? 'Déjà ajouté' : 'Système';
                column.appendChild(article);
                ui.catalogContainer.appendChild(column);
                return;
            }
    ...
    const renderActiveBlocks = () => {
        ...
        blocks.forEach((block, index) => {
            ...
            article.querySelector('.te-block-id').textContent = `${block.id} #${index + 1}`;
    ...
    const updateListItemTitles = (listEditor) => {
        Array.from(listEditor.querySelectorAll('[data-list-item]')).forEach((itemNode, index) => {
            ...
            if (titleNode) {
                titleNode.textContent = `Élément #${index + 1}`;
            }
        });
    };
    ...
    const openBlockModal = (index) => {
        ...
        ui.modalTitle.textContent = `Configurer: ${catalogItem.label}`;
        ui.modalBody.innerHTML = '';
        ...
        if (formTemplate instanceof HTMLTemplateElement) {
            ...
            empty.textContent = 'Aucun formulaire disponible pour ce bloc.';
...
`;
    ...
    const ensureImagePreviewNode = (select) => {
        ...
        if (!preview) {
            ...
            caption.textContent = 'Aucune image sélectionnée';
    ...
    const updateImagePreviewForSelect = (select) => {
        ...
        if (image) {
            image.src = imageUrl;
            image.alt = `Prévisualisation ${rawValue}`;
        }
        if (caption) {
            caption.textContent = rawValue;
        }
    ...
    const updateBlockDomPreview = (routeIndex) => {
        ...
        const applyHeaderButtonStyles = (headerShell, rules, serverAddress) => {
            ...
            links.forEach((link) => {
                ...
                if (variant === 'server') {
                    ...
                    link.textContent = resolvedAddress || 'Serveur indisponible';
        ...
        const applyLandingButton = (node, options) => {
            ...
            const text = variant === 'server'
                ? (serverAddress || 'Serveur indisponible')
                : String(options.text || options.fallbackText || 'En savoir plus');
            ...
            if (variant === 'server') {
                ...
                node.setAttribute('data-bs-title', 'Copié !');
            } else {

...
@section('content')
    <div class="container mx-auto">
        <div class="card md:py-10">
            <div class="alert alert-danger text-red-500 hidden" id="errorAlert" role="status">
                <p>{{ trans('shop::messages.payment.error') }}</p>
                <span id="errorMessage"></span>
            </div>

            <div class="flex justify-between flex-col items-center md:flex-row gap-8 flex-wrap">
                <div>
                    <div class="flex flex-col items-center gap-4 mb-8">
                        <h1 class="text-4xl">PayPal Checkout</h1>
                        <span class="rounded-full px-5 py-1 bg-green-500 text-2xl" style="background: green">{{ $payment->formatPrice() }}</span>
                    </div>

                    <a href="{{ route('shop.home') }}" class="btn btn-primary">
                        {{ trans('messages.actions.cancel') }} <i class="bi bi-x-lg"></i>
                    </a>
                </div>

                <div id="pay-buttons" class="position-relative mb-1 w-full md:w-1/2">

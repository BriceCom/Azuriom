<div class="copyright">
    <div class="container text-center text-md-start">
        <div class="row gy-3">
            <div class="col-md-6">
                <h2 class="fs-4 text-white">{{theme_config('footer.left.title') ?? 'About'}}</h2>
            ...
            <div class="col-md-3">
                <h2 class="fs-4 text-white">{{theme_config('footer.middle.title') ?? 'Useful links'}}</h2>
                <ul class="list-unstyled m-0 text-white-50">
                    @if(theme_config('footer.center.link') != null)
                        @foreach(theme_config('footer.center.link') as $link)
                            <li class="py-1"><a class="text-white opacity-50 text-decoration-none" href="{{$link['url'] ?? '#'}}"><i class="{{$link['icon'] ?? 'bi bi-house'}} me-2"></i>{{$link['text'] ?? 'Home'}}</a></li>
                        @endforeach
                    @else
            ...
            <div class="col-md-3 d-flex flex-column">
                <h2 class="fs-4 text-white">{{theme_config('footer.right.title') ?? 'Support us'}}</h2>

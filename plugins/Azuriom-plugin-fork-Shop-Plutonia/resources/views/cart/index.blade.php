...
@section('content')
        ...
        <div class="card-body">
                        ...
                        <table class="table cart-items">
                            ...
                            @foreach($cart->content() as $cartItem)
                                <tr>
                                    ...
                                    <td class="text-nowrap">{{ shop_format_amount($cartItem->price()) }}
                                        @if(use_site_money() && $alternativeCurrency)
                                            ou {{$cartItem->alternativePrice()}} {{ac_shop_currency_name()}}
                                        @endif
                                    </td>
                                    <td class="text-nowrap">{{ shop_format_amount($cartItem->total()) }}
                                        @if(use_site_money() && $alternativeCurrency )
                                            ou {{ $cartItem->totalAlternative() .' '. ac_shop_currency_name() }}
                                        @endif
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <a href="{{ route('shop.cart.remove', $cartItem->id) }}"
                                           class="btn btn-sm btn-danger" title="{{ trans('messages.actions.delete') }}">
            ...
            <div class="row">
                <div class="col-md-5">
                    ...
                    @if(use_site_money())
                        <p>Seul les paiements en {{money_name()}} sont éligibles aux coupons</p>
                    @endif
            ...
            <h5 class="text-end">
                ...
                @if($alternativeCurrency)
                    <br>ou <br>
                    {{$cart->totalAlternative()}} {{ac_shop_currency_name()}}
                @endif
            </h5>
            ...
            <div class="row">
                <div class="col-md-5">
                    ...
                    @if(use_site_money())
                        <p>Seul les paiements en {{money_name()}} sont éligibles aux carte cadeaux</p>
                    @endif
    ...
    @if(use_site_money())
        <div class="modal modal-lg fade" id="confirmBuyModal" tabindex="-1" role="dialog"
             aria-labelledby="confirmBuyLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex flex-column gap-3">
                                            <p>{{ Auth::user()->money . ' ' . money_name()}}
                                                @if($alternativeCurrency)
                                                    <br>-<br>{{$userAlternativeCurrency->amount}} {{ac_shop_currency_name()}}
                                                @endif
                                            </p>
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex flex-column gap-3">
                                            <p>{{ shop_format_amount($cart->payableTotal()) }}
                                                @if($alternativeCurrency)
                                                    <br>ou <br>{{$cart->totalAlternative()}} {{ac_shop_currency_name()}}
                                                @endif
                                            </p>
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    <td>Solde après payement :</td>
                                    <td>
                                        <div class="d-flex flex-column gap-3">
                                            <p>{{ Auth::user()->money - $cart->payableTotal(). ' ' . money_name() }}
                                                @if($alternativeCurrency)
                                                    <br>- <br>
                                                    {{$userAlternativeCurrency->amount - $cart->totalAlternative()}} {{ac_shop_currency_name()}}
                                                @endif
                                            </p>
                                        </div>
                                    </td>
                        ...
                        <p class="alert alert-info text-body-emphasis">
                            <i class="bi bi-info-circle-fill"></i>
                            Si vous achetez avec des {{money_name()}} tous les articles s'achèterons par cette monnaie. @if($alternativeCurrency)Si vous achetez avec {{ac_shop_currency_name()}} seulement les articles éligibles seront acheté.@endif
                        </p>
                    </div>
                    ...
                    <div class="modal-footer">
                        ...
                        <div class="ms-auto d-flex gap-2">
                            @if($alternativeCurrency)
                                <form method="POST" action="{{ route('shop.cart.alternativePayment') }}">
                                    @csrf
                                    <button type="submit" class="btn btn-primary ms-auto" data-bs-toggle="modal"
                                            data-bs-target="#confirmBuyModal">
                                        {{ trans('shop::messages.cart.pay') }} en {{ac_shop_currency_name()}}
                                    </button>
                                </form>
                            @endif
                            ...
                            <form method="POST" action="{{ route('shop.cart.payment') }}">
                                ...
                                <button type="submit" class="btn btn-primary ms-auto" data-bs-toggle="modal"
                                        data-bs-target="#confirmBuyModal">
                                    {{ trans('shop::messages.cart.pay') }} en {{money_name()}}
                                </button>
                            </form>
                        </div>
                    </div>

                    ...
                    <div class="col-md-4">
                        <div class="card h-100">
                            ...
                            <div class="card-body d-flex flex-column justify-content-between">
                                <h4 class="flex-grow-1 card-title h3">{{ $package->name }}</h4>
                                <h5 class="card-subtitle mb-3 fw-bold">
                                    @if($package->isDiscounted())
                                        <del class="text-1xl text-danger">{{ $package->getOriginalPrice() }}</del>
                                    @endif
                                    <span class="text-1xl text-tertiary">{{ $package->getPrice() }}</span>
                                    <small class="text-sm text-tertiary">{{ shop_active_currency() }}</small>
                                </h5>

                                <a href="#" class="w-100 btn btn-outline-primary" data-package-url="{{ route('shop.packages.show', $package) }}">

...
<head>
    ...
    @stack('styles')
    <style>
        @import url({{theme_config('style.index.font.url') ?? 'https://fonts.bunny.net/css?family=rubik:200,300,400,500,600,700,800&display=swap'}});

        :root {
            --di-main-font: "{{theme_config('style.index.font.name') ?? 'rubik'}}", sans-serif;
        }
    </style>
    ...
    <script src="{{ theme_asset('js/counter.js') }}" defer></script>
...
<div id="app" class="d-flex flex-column">
    ...
    <div class="flex-grow-1 overflow-x-hidden d-flex flex-column">
        @yield('app')
...
<script>
    document.querySelectorAll('.card, .faq-section, .carousel-section').forEach((el, index) => {

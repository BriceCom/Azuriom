@if(!theme_config('sidebar.vote.off'))
    @if(plugins()->isEnabled('vote'))
        <div class="home-sideblock home-sideblock-vote">
            <h2 class="h3 mb-3">{{ theme_config('sidebar.vote.title') ?? trans('theme::theme.top_voters')}}</h2>
            ...
            <a href="{{ route('vote.home') }}"
               class="btn btn-primary w-100">{{ trans('vote::messages.sections.vote')}}</a>
        </div>
    @endif
@endif

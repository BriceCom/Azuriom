...
<div id="{{ $id }}-input" data-listInput="true">
    ...
    @empty
        <div class="input-group mb-2 gap-2">
            ...
            @if($inputUrl)
                <input type="url" class="form-control" placeholder="{{ trans('messages.fields.link') }}"
                       name="{{ $name }}[{index}][href]">
            @endif


            @if($inputColor)
                @include('admin.components.forms.color', ['name' => trans('theme::admin.form.color.color'), 'id' => $name.'[{index}][color]', 'hideLabel' => true])
            @endif
            ...
            @if($inputTexarea)
                <textarea class="w-100 form-control" name="{{ $name }}[{index}][content]"></textarea>
            @endif

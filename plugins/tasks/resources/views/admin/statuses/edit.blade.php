...
@section('title', trans('tasks::admin.statuses.edit'))

@section('content')
    <div class="card shadow mb-4">
        <div class="card-header">
            <h6 class="m-0 font-weight-bold text-primary">{{ trans('tasks::admin.statuses.edit') }}</h6>
        </div>
        <div class="card-body">
            <form action="{{ route('tasks.admin.statuses.update', $status) }}" method="POST">
                @csrf
                ...
                <div class="mb-3">
                    ...
                    <div class="input-group">
                        <span class="input-group-text">
                            <input type="color" id="color-picker" value="{{ old('color', $status->color) }}" class="form-control form-control-color p-0 border-0">
                        </span>
                        <input type="text" class="form-control @error('color') is-invalid @enderror" id="color" name="color" value="{{ old('color', $status->color) }}" pattern="^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$" required>
                        @error('color')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>

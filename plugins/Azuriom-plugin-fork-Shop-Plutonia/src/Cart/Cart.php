...
class Cart implements Arrayable
{
     ...
     * applying discounts and percentage coupons.
     */
    public function totalAlternative(): float
    {
        return $this->itemsPrice()->sum(fn (array $item) => $item['alternative_price']);
    }
        ...
        return $this->content()->map(function (CartItem $item) use ($remaining) {
            ...
            if (! $package instanceof Package) {
                return [
                    'item' => $item,
                    'price' => $item->total(),
                    'alternative_price' => $package->alternative_price,
            ...
            return [
                'item' => $item,
                'price' => $total,
                'alternative_price' => $package->alternative_price * $item->quantity,

                    ...
                    <div class="flex justify-center items-center w-auto overflow-hidden gap-2">
                        ...
                        <div class="w-full text-ellipsis truncate">
                            <h4 class="text-white font-medium truncate md:text-sm text-xs">{{
                                trans('messages.profile.2fa.title') }}</h4>
                        </div>
                    </div>
                </div>
                <div class="p-8 grid xl:grid-cols-2  grid-cols-1 gap-8">
                    <div
                        class="w-64 h-[28em] rounded-[28px] border-[8px] border-steel-100 relative overflow-hidden shadow-2xl bg-steel-300 mx-auto">
                        <div
                            class="px-4 relative overflow-hidden flex flex-col flex justify-center items-center h-full">
                            <div>
                                <div
                                    class="bg-steel-200 relative border-corner relative m-auto rounded-xl bg-cover flex cursor-none qr-code-block w-44 h-44">
                                    {{ $qrCode }}
                                </div>
                                <p class="text-white text-xs mt-3 w-56 text-center">@lang('messages.profile.2fa.info')
                                </p>
                                <div class="justify-center items-center flex flex-cols space-x-3 mt-2">
                                    <img class="h-8 w-8 rounded-lg" src="{{ theme_asset('img/2fa_app1.webp') }}">
                                    <img class="h-8 w-8 rounded-lg" src="{{ theme_asset('img/2fa_app2.webp') }}">
                                    <img class="h-8 w-8 rounded-lg" src="{{ theme_asset('img/2fa_app3.webp') }}">
                                </div>
                                <p class="text-white text-center text-xs font-medium px-4 mt-2">
                                    @lang('messages.profile.2fa.secret', ['secret' => $secret])</p>
                            </div>
                        </div>
                        <div class="absolute bottom-0 inset-x-0 h-8">
                            <div class="absolute bottom-1 inset-x-0 mb-2">
                                <div class="mx-auto h-1 w-28 rounded bg-white"></div>
                            </div>
                        </div>
                    </div>
                    <form method="POST" action="{{ route('profile.2fa.enable') }}"
                        class="bg-steel-200 flex flex-col gap-8 my-auto">
                        @csrf

                        <input type="hidden" name="2fa_key" value="{{ $secret }}">

                        <div class="relative bg-inherit">
                            <input type="text" id="codeInput" name="code" minlength="1" maxlength="6"
                                placeholder="{{ trans('messages.profile.2fa.code') }}"
                                class="peer bg-transparent h-14 w-full rounded-xl text-white placeholder-transparent px-4 border-steel-100 focus:border-white focus:outline-none font-display font-medium text-sm border transition duration-300 @error('code') is-invalid @enderror">
                            <label for="codeInput"
                                ...
                                trans('messages.profile.2fa.code') }}</label>
                        </div>
                        <button type="submit" class="bg-steel-100 px-3 py-4 rounded-xl text-white h-14 text-sm truncate"
                            data-ripple-dark="true">
                            {{ trans('messages.actions.enable') }}
                        </button>
                        <a class="bg-danger px-3 py-4 rounded-xl text-white h-14 text-sm truncate text-center"

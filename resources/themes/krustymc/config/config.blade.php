...
@php
    ...
    $menus = [
        ...
        'home' => [
            ...
            'is_enabled' => true,
            'multiple_page' => [
                'join' =>['name'=> trans('theme::admin.menus.home.join')],
                'stats' =>['name'=> trans('theme::admin.menus.home.stats')],
            ]
        ],
        'footer' => [
            'not_a_plugin' => true,
                        ...
                        <div class="tab-pane fade show {{$key == 'settings' ? 'active':''}}" id="v-pills-{{$key}}" role="tabpanel" aria-labelledby="v-pills-{{$key}}-tab" tabindex="0">
                                @if(isset($menu['multiple_page']))
                                    <div>
                                        ...
                                        <div class="px-2">
                                            @foreach($menu['multiple_page'] as $keyPage=>$multi_page)
                                                ...
                                                @endif
                                            @endforeach
                                            <button class="btn btn-outline-primary" type="button" data-bs-toggle="collapse" data-bs-target=".multi-collapse" aria-expanded="true" aria-controls="">
                                    ...
                                    <div class="mt-3">
                                        @foreach($menu['multiple_page'] as $keyPage=>$multi_page)
                                            @if(View::exists('admin.forms.plugins.'.$key.'.'.$keyPage))
                                                <div class="collapse multi-collapse show {{$keyPage == 'index' ? 'show':''}}" id="{{$keyPage}}">
                                                    <div class="card">

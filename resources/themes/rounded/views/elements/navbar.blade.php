<nav class="navbar navbar-expand-md bg-dark py-3 py-md-0 px-3 rounded-3">
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="navbar">
            <!-- Left Side Of Navbar -->
            <ul class="navigation navbar-nav align-items-md-center py-3 py-md-0 gap-3 me-auto list-unstyled">
                @foreach($navbar as $element)
                    @if(!$element->isDropdown())
                        <li class="nav-item">
                            <a class="nav-link @if($element->isCurrent()) active @endif @if(str_contains($element->name, (theme_config('navbar.button.text') ?? 'boutique'))) custom-link text-white px-4 rounded-3 @endif" @if(strtolower($element->name) === 'boutique') @endif   href="{{ $element->getLink() }}" @if($element->new_tab) rel="noopener noreferrer" @endif>
                                {{ $element->name }}
                            </a>
                        </li>
                    @else
                        <li class="nav-item dropdown">
                            <a class="nav-link py-4 dropdown-toggle @if($element->isCurrent()) active @endif" href="#" id="navbarDropdown{{ $element->id }}" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                {{ $element->name }}
                            </a>
            ...
            <ul class="navbar-nav ml-auto">
                @if(plugins()->isEnabled('shop'))
                    @if(!theme_config('hero.basket.toggle'))
                        @if(theme_config('general.serveurliste'))

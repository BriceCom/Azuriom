...
<head>
    ...
    <style>
        @if(theme_config('premium.serveurliste.link') && theme_config('style.index.font.on'))
               @import url({{theme_config('style.index.font.url') ?? 'https://fonts.bunny.net/css?family=saira:100,200,300,400,500,600,700,800,900&display=swap'}});

        body {
            font-family: "{{theme_config('style.index.font.name') ?? 'saira'}}", sans-serif;
        }
        @endif
    </style>
    <script type="text/javascript">

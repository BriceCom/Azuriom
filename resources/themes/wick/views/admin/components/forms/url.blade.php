...
<div class="form-group">
    ...
    <input id="{{ $idReplace }}" name="{{ $id }}" type="url" placeholder="{{$placeholder ?? ''}}" class="form-control @error($id) is-invalid @enderror" value="{{ old($idReplace, config('theme.'.str_replace(['[', ']'],['.',''],$id))) }}" aria-describedby="{{$id}}">
    @error($id)

    ...
    <div>
        <form class="w-100" action="{{ route('admin.themes.config', $theme) }}" method="POST">
            ...
            <div class="alert alert-warning w-100 mb-3">
                ...
                <input pattern="^https:\/\/www\.serveurliste\.com\/.*" placeholder="https://www.serveurliste.com/minecraft/your_server" type="url" class="form-control @error('block-button-text') is-invalid @enderror" id="block-button-text" name="general[serveurliste]" value="{{old('general-serverliste', config('theme.general.serveurliste'))}}" aria-describedby="block-button-text-Label">
            ...
            <div class="card bg-secondary bg-opacity-10">
                <div class="card-header bg-secondary bg-opacity-25">
                    <h2 class="fw-bold fs-3 m-0 text-uppercase">{{trans('theme::admin.navbar')}}</h2>
                </div>

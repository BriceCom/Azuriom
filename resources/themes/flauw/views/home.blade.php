...
@section('app')
    @php
        $launcherMode = theme_config('hero.server.launcher.on') && theme_config('hero.server.launcher.url');
    @endphp
    <div class="home-background d-flex align-items-center justify-content-end flex-column text-white pb-md-15 pb-3 " style="background: rgb(87,110,247);background: radial-gradient(circle, rgba(87,110,247,0) 6%, rgba(var(--bs-black-rgb),0.6) 98%),linear-gradient(rgba(18,23,37,0.79), rgba(var(--bs-light-rgb),0.8)),url('{{ setting('background') ? image_url(setting('background')) : 'https://via.placeholder.com/2000x500' }}') no-repeat center; background-size: cover">
        ...
        <div class="container d-flex flex-column flex-md-row justify-content-center align-items-center gap-md-4">
            @if($launcherMode)
                <a href="{{ theme_config('hero.server.launcher.url') }}" class="hero-small-box d-flex align-items-center gap-3 bg-dark bg-opacity-25 p-3 text-white text-decoration-none">
                    <div>
                        <h2 class="m-0 fs-5">{{ theme_config('hero.server.launcher.text') ?? 'PLAY' }}</h2>
                    </div>
                ...
                <div data-clipboard-text="{{theme_config('hero.server.ip') ?? 'play.dixept.fr'}}" class="copy_ip hero-small-box d-flex align-items-center gap-3 bg-dark bg-opacity-25 p-3" style="cursor : pointer;">
                    <div>
                        <h2 class="m-0 fs-5 server_count">
                            ...
                            @else
                                {{theme_config('hero.server.text') ?? 'SERVER OFFLINE'}}
                            @endif
                        </h2>
            ...
            <a href="{{theme_config('hero.discord.url') ?? 'https://discord.gg/Gh2ddyBxUWvV'}}" target="_blank" class="hero-small-box d-flex align-items-center gap-3 bg-dark bg-opacity-25 p-3 text-white text-decoration-none">
                ...
                <div>
                    <h2 class="m-0 fs-5 discord-list_count"><span>{{theme_config('hero.discord.text') ?? 'USERS ONLINE'}}</span></h2>
    ...
    <div class="bg-body content">
        ...
        <div class="container">
                ...
                <div class="container py-8 pb-2">
                    <div class="d-flex flex-column flex-md-row gap-md-4 bg-secondary bg-opacity-75 mx-md-8">
                        ...
                        <div class="d-flex flex-column text-md-start text-center py-3" style="overflow-wrap: anywhere;">
                            <h2 class="m-0 fw-bold fs-4">{{theme_config('event.box.title') ?? 'EVENT:'}}</h2>{{theme_config('event.box.paragraph') ?? 'Lorem ipsum dolor sit amet, consectetur adipisicing elit. Alias aliquam animi deleniti distinctio dolor doloribus eos esse, maiores pariatur sint?'}}
            ...
            @php
                $showSidebarButton = !theme_config('block.button.toggle');
                $showSidebarDiscord = !theme_config('block.discord.toggle');
                $showSidebarTwitter = !theme_config('block.twitter.toggle');
                $hasSidebar = $showSidebarButton || $showSidebarDiscord || $showSidebarTwitter;
            @endphp
            <div class="row py-8 {{ $hasSidebar ? '' : 'justify-content-center' }}">
                @if($hasSidebar)
                ...
                @endif
                <div class="{{ $hasSidebar ? 'col-md-9' : 'col-md-10' }} p-0">
                    @if(! $posts->isEmpty())
                        <div class="bg-dark text-center py-3 border-bottom-0 {{ $hasSidebar ? 'rounded-end' : 'rounded' }}">

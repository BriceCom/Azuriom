...
@section('content')
    <section class="page-top">
        <h2>{{ trans('messages.profile.title') }}</h2>
        <div class="block"></div>
    </section>
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-xl-2 col-md-3 text-center">
                    ...
                    <h3 class="h5 mb-0">
                        <span class="badge"
                              style="{{ $user->role->getBadgeStyle() }}; vertical-align: middle">{{ $user->role->name }}</span>
                    </h3>
                </div>
                ...
                <div class="col-lx-10 col-md-9">
                    ...
                    <ul>
                        ...
                        @if(use_site_money() && plugins()->isEnabled('alternative-currency') === true)
                            @php
                                $alternativeCurrencyUser = \Azuriom\Plugin\AlternativeCurrency\Models\User::whereUserId($user->id)
                                    ->whereHas('coin', function ($query) {
                                        $query->whereShopCurrency(true);
                                    })
                                    ->with('coin')
                                    ->first();
                            @endphp
                            <li>Argent alternatif: {{ $alternativeCurrencyUser->amount }} {{ $alternativeCurrencyUser->coin->name }}</li>
                        @endif
                        @if(! oauth_login())
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form action="{{ route('profile.transfer-money') }}" method="POST">
                                ...
                                <div class="mb-3">
                                    <label class="form-label"
                                           for="moneyInput">{{ trans('messages.fields.money') }}</label>

...
if (! function_exists('ac_shop_currency_name')) {
    /**
     * Return the money name for the given amount.
     */
    function ac_shop_currency_name(): string
    {
        if(plugins()->isEnabled('alternative-currency')){
            $coinName = Azuriom\Plugin\AlternativeCurrency\Models\Coin::whereShopCurrency(true)->first()->name;
...
if (! function_exists('ac_shop_currency_id')) {
    /**
     * Return the money name for the given amount.
     */
    function ac_shop_currency_id(): string
    {
        if(plugins()->isEnabled('alternative-currency')){
            $coin = Azuriom\Plugin\AlternativeCurrency\Models\Coin::whereShopCurrency(true)->first()->id;

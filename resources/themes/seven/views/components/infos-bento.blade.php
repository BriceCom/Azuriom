    <section class="d-flex flex-column gap-4">
        <div class="row w-100 mx-auto gap-4 gap-md-0">
                    ...
                    <div class="card-body d-flex flex-column gap-3 align-items-baseline">
                        ...
                        @if(plugins()->isEnabled('vote'))
                            ...
                            <div class="flex-grow-1 w-100">
                                <div
                                    class="d-flex flex-column flex-md-row align-items-center align-items-md-start justify-content-md-around"
                                    style="height: auto;">
                                    @forelse($votes->take(3) as $id => $vote)
                                        <div
                                            ...
                                            @else
                                                <img class="mb-2" src="{{$vote->user->getAvatar()}}"
                                                     alt="Avatar {{$vote->user->name}}">
                                            @endif
                                            <span class="badge"
                            ...
                            <a href="/vote"
                               class="btn btn-tertiary text-uppercase mt-2 ms-auto">{{trans('theme::theme.vote_for', ['name' => site_name()])}}
            ...
            <div class="col-md-6 ps-md-2 p-0 d-flex flex-column gap-4">
                    ...
                    @guest
                        <div class="card-body">
                            ...
                            <div class="d-flex flex-column flex-md-row align-items-end gap-3 mt-4">
                                <a href="{{route('login')}}"
                                   class="btn btn-tertiary text-uppercase">{{ trans('auth.login') }} <i
                                        class="bi bi-arrow-right"></i></a>
                                @if(Route::has('register'))
                                    <a href="{{route('register')}}">{{trans('theme::theme.dont_have_account')}}</a>
                                @endif
                            </div>
                        </div>
                    @else
                        <div class="card-body d-flex align-items-center justify-content-between gap-2">
                            <div class="infosbento__avatar">
                                <img src="{{Auth::user()->getAvatar(64)}}" class="object-fit-contain rounded-2"
                                     height="64" alt="">
                            </div>
                            <div class="d-flex flex-column align-items-end gap-2">
                                <span class="d-block badge"
                                      style="background-color: {{Auth::user()->role->color}};">{{Auth::user()->role->name}}</span>
                                @if(plugins()->isEnabled('vote'))
                                    <p class="m-0 text-sm">{{\Azuriom\Plugin\Vote\Models\Vote::where('user_id', Auth::id())->where('created_at', '>=', now()->startOfMonth())->count()}}
                                        votes</p>
                                @endif
                                ...
                                <a href="{{route('profile.index')}}"
                                   class="btn btn-tertiary text-uppercase mt-2">{{trans('messages.nav.profile')}} <i
                                        class="bi bi-arrow-right"></i></a>
                            </div>
                        </div>
                    @endguest
                ...
                <div class="card">
                    <div class="card-body">
                        <div
                            class="d-flex justify-content-center align-items-center justify-content-between flex-wrap gap-md-3">
                            <div>
                                <img src="{{site_logo()}}" alt="Logo" class="object-fit-contain me-md-4" width="70">
                            </div>
                            <div class="d-flex align-items-center gap-3">
                                ...
                                <button
                                    ...
                                    aria-label="{{trans('theme::theme.server_address_copied')}}"
        ...
        <div class="card">
            <div class="card-body text-center py-4">
                ...
                <a href="/shop"
                   class="btn btn-secondary mx-auto mt-1 text-sm text-uppercase">{{theme_config('home.bentobox.shop.link.text') ?? "Ipsum"}}</a>
            </div>
        </div>
    </section>

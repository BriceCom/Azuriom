...
Le layout parent doit relire les valeurs partagées avec `view()->shared(...)` avant d’utiliser `teHeaderView`, `teFooterView`, `teLayoutBlockParams` et `teThemeEditorVersion`, car un `@include(...)` ne remonte pas ses variables dans le scope du template appelant.
...
- le core charge en option [registry/demo-catalog.php](/Applications/MAMP/htdocs/azuriom_neuf/resources/themes/bird/views/theme-editor/registry/demo-catalog.php) pour les blocs Bird spécifiques
- `default_blocks_for_route`: blocs par défaut par route, via [registry/default-blocks.php](/Applications/MAMP/htdocs/azuriom_neuf/resources/themes/bird/views/theme-editor/registry/default-blocks.php) si présent
...
Dans l'onglet `Page builder`, la zone `Presets JSON` permet d'exporter et de réimporter l'état courant du thème, y compris les blocs de page.
La liste des blocs accepte le glisser-déposer, et les libellés visibles de l'éditeur passent par les traductions du thème.
Les boutons `Annuler` et `Rétablir` du même onglet pilotent l'historique des blocs de la route courante.

<nav class="navbar navbar-expand-lg py-3">
    <div class="container">
        <a class="navbar-brand d-md-none" href="{{ route('home') }}">
            <img height="57" src="{{site_logo()}}" alt="Logo de {{site_name()}}">
        </a>
        <button class="navbar-toggler border-0" type="button" data-bs-toggle="collapse" data-bs-target="#navbar" aria-controls="navbar" aria-expanded="false" aria-label="{{ trans('messages.nav.toggle') }}">
            <span class="navbar-toggler-icon"></span>
        </button>

        <div class="collapse navbar-collapse" id="navbar">
            <!-- Left Side Of Navbar -->
            <ul class="w-100 navbar-nav align-items-md-center gap-md-7 list-unstyled mt-4 mt-md-0">
                @foreach($navbar as $element)
                    @if(!$element->isDropdown())
                        <li class="nav-item @if($loop->last) ms-md-auto me-3 @endif @if(str_contains(strtolower($element->name), 'boutique')) custom-link @endif">
...
{{--                    @include('elements.notifications')--}}
                    ...
                    <li class="nav-item dropdown">
                        <a id="userDropdown" class="nav-link dropdown-toggle" href="#" role="button" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false" v-pre>
                            <img class="rounded-pill" src="{{ Auth::user()->getAvatar(50) }}" alt="{{ Auth::user()->name }}">
                        </a>
...
@push('footer-scripts')
    <script>
        ...
        window.addEventListener('scroll', () => {
            if (!header.classList.contains('scroll') && window.scrollY > 0) {
                header.classList.add('scroll');
            } else if (header.classList.contains('scroll') && window.scrollY === 0) {
